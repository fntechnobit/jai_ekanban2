...
@section('title', 'History Print Shikake')

@section('breadcrumb')
    <x-page-header menu-code="ekanban_shikake_history" />
@endsection
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h3 class="card-title mb-0">
                    <i class="fa-solid fa-clock-rotate-left"></i> History Print Shikake
                </h3>
...
@section('script')
    <script>
        ...
        var FILTER_STORAGE_KEY = 'ekanban_shikake_history_filters';
        ...
        $(function () {
            ...
            // options are fetched for the restored area + process
            if (savedFilters) {
                ...
                $('#filter_process, #filter_area, #filter_shift_co').trigger('change.select2');
            }
            ...
            table = $('#history-table').DataTable({
                ...
                ajax: {
                    url: "{{ route('schedule.ekanban-shikake.history') }}",
                    data: function (d) {
                    ...
                    error: function (xhr) {
                        ...
                        $('#history-table tbody').html(
                            '<tr><td colspan="16" class="text-center text-danger">' +
            ...
            // Area + process drive the machine list - refresh its options first and
            // only then reload, so the table never queries a stale machine
            $('#filter_area, #filter_process').on('change', function () {
                loadMachines($('#filter_area').val(), $('#filter_process').val(), { autoReload: true });
            });
            ...
            $('#btn-reset').click(function () {
                clearFilters();
                $('#filter_process').val('all').trigger('change.select2');
            ...
            $('#btn-export').click(function () {
                ...
                window.location.href = "{{ route('schedule.ekanban-shikake.history.export') }}?" + params;
            });

            // Machine options follow area (and process) - "all" stays available.
            ...
            function loadMachines(areaId, process, opts) {
                opts = opts || {};
                ...
                $.ajax({
                    ...
                    type: 'GET',
                    data: { area_id: areaId, process: process },
            ...
            // Restore the machine options for the saved area + process, then run
            // the first query (the table was created with deferLoading)
            loadMachines($('#filter_area').val(), $('#filter_process').val(), {
