{{-- CIRCUIT TWIST Print Template - PRINT VERSION (landscape, 80mm height) --}}
<style>
...
.twist-print-wrapper .circuit-twist-image-section {
    flex-shrink: 0;
    display: flex;
    align-items: center;
...
.twist-print-wrapper .circuit-twist-image-section img {
    height: 100%;
    width: auto;
    object-fit: contain;
}
...
<div class="ticket twist-print-wrapper" data-orientation="landscape">
    ...
    <div class="circuit-twist-image-section">
    ...
    <table>
        ...
        <tbody>
            ...
            <tr class="twist-label-cell">
                ...
                <td colspan="3" rowspan="2" class="twist-barcode-cell">
                    @if(isset($circuit->barcode_navigasi_path))
                        <img src="{{ $circuit->barcode_navigasi_path }}" alt="Barcode"><br>
            ...
            <tr class="twist-value-cell">
                <td colspan="2">{{ $circuit->cct_code ?? '-' }}</td>
                <td>{{ $circuit->cct_no ?? '-' }}</td>
                <td colspan="2">{{ $circuit->machine ?? '-' }}</td>
            ...
            <tr class="twist-value-cell">
                <td colspan="2">{{ $circuit->cust_no ?? '-' }}</td>
                <td>{{ $circuit->kind ?? '-' }}</td>
                <td>{{ $circuit->size ?? '-' }}</td>
                <td>{{ $circuit->col ?? '-' }}</td>
                <td>{{ $circuit->cl ?? '-' }}</td>
                <td>{{ $circuit->qty ?? '-' }}</td>
                <td colspan="2">{{ $circuit->issue ?? '-' }}</td>
            </tr>
            ...
            <tr>
                ...
                <td class="twist-label-cell">MACH. TWIST</td>
                <td class="twist-label-cell">MEMORY TWIST</td>
            ...
            <tr>
                ...
                <td class="twist-value-cell">{{ $circuit->machine_twist ?? '-' }}</td>
            ...
            <tr>
                ...
                <td colspan="3" rowspan="2" class="twist-barcode-cell">
                    @if(isset($circuit->barcode_process_path))
                        <img src="{{ $circuit->barcode_process_path }}" alt="Barcode"><br>
            ...
            <tr>
                <td class="twist-label-cell text-left">ACC 1</td>
                <td colspan="2" class="twist-value-cell text-left">{{ $circuit->acc_1b ?? '-' }}</td>
                <td class="twist-label-cell text-left">STRIP</td>
