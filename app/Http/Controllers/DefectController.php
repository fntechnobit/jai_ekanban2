...
class DefectController extends Controller
{
    ...
    public function cuttingImportStoPreview(Request $request)
    {
        ...
        try {
            $result = $this->defectService->previewCuttingSto(
            ...
            Log::error('DefectController: cuttingImportStoPreview failed', ['error' => $e->getMessage()]);
            return response()->json([
                'success' => false,
    ...
    /**
     * Apply one chunk of previously previewed STO rows as balance-reducing
     * defects (AJAX). The frontend calls this repeatedly, one chunk at a
     * time, to drive an incremental progress bar.
     */
    public function cuttingImportStoCommit(Request $request)
    {
        ...
        try {
            $result = $this->defectService->commitCuttingDefects(
                (int) $validated['conveyor_id'],
                $validated['defect_date'],
                (int) $validated['shift'],
                $validated['items']
            );
            ...
            Log::error('DefectController: cuttingImportStoCommit failed', ['error' => $e->getMessage()]);
            return response()->json([
                'success' => false,
                'message' => $e->getMessage(),
            ], 422);
